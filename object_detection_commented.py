...
def detect(frame, net, transform): # Define a detect function that will take as inputs, a frame, a ssd neural network, and a transformation to be applied on the images, and that will return the frame with the detector rectangle.
    ...
    detections = y.data # Create the detections tensor contained in the output y.
    ...
    for i in range(detections.size(1)): # For every class:
        ...
        while detections[0, i, j, 0] >= 0.6: # Take into account all the occurrences j of the class i that have a matching score larger than 0.6.
            ...
            cv2.putText(frame, labelmap[i - 1], (int(pt[0]), int(pt[1])), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 2, cv2.LINE_AA) # put the label of the class right above the rectangle.
...
net.load_state_dict(torch.load('ssd300_mAP_77.43_v2.pth', map_location = lambda storage, loc: storage)) # get the weights of the neural network from another one that is pretrained (ssd300_mAP_77.43_v2.pth).
...
reader = imageio.get_reader('funny_dog.mp4') # open the video.
fps = reader.get_meta_data()['fps'] # get the fps frequence (frames per second).
writer = imageio.get_writer('output.mp4', fps = fps) # create an output video with this same fps frequence.
for i, frame in enumerate(reader): # iterate on the frames of the output video:
    frame = detect(frame, net.eval(), transform) # call our detect function (defined above) to detect the object on the frame.
